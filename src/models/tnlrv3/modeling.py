...
class BertSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, rel_pos=None):
        ...
        if self.sparse:
            attention_mask = self.transpose_mask_for_sparse(
                attention_mask, hidden_states.shape[0]
            )

            attention_mask = attention_mask.to(query_layer.dtype)
            if self.rel_pos_type in (1, 2):
                rel_pos = rel_pos.to(query_layer.dtype).contiguous()
            ...
            ).masked_fill_(attention_mask.bool(), float('-inf'))
